...
/* ---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------- */
	...
	void BESSY (int N, double X,double y[]) {
/* -----------------------------------------------------------------
      This subroutine calculates the second kind Bessel Function of
  ...
  ------------------------------------------------------------------ */
    double TOX,BY,BYM,BYP; int J;
      if (N == 0){ y[0]=BESSY0(X); return ;}
      if (N == 1)  {y[0] = BESSY1(X); return;};
      if (X == 0.0) {y[0]= -1e30; return;}
      TOX = 2.0/X;
      BY  = BESSY1(X);
      BYM = BESSY0(X);
      for (J=1; J<N; J++) {
