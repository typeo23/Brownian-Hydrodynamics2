function D = BuildDiffusionMatrixPeriodic(parts,Eta_m,Kappa,kT,boxSize)
...
for i=1:2:2*N
    for j=(i+2):2:2*N
        Dtmp = BuildDiffusionMatrixInteraction(parts(round(i/2),:),parts(round(j/2),:),Eta_m,Kappa,kT,boxSize);
        D(i:(i+1),j:(j+1)) = Dtmp;
        D(j:(j+1),i:(i+1)) = Dtmp;
    end
end
